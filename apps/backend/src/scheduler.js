...
import { sendSessionSummary } from "./mailer.js";
...
async function tick() {
  ...
  for (const session of toFinalize) {
    ...
    const [finalizedSession] = await prisma.$transaction([
    ...
    // Send session summary email (non-blocking)
    const user = await prisma.user.findUnique({
      where: { id: session.booking.userId },
      select: { fullName: true, email: true },
    });
    sendSessionSummary(user, { ...finalizedSession, endedAt: session.booking.endAt, energyKwh, costAud }, session.booking, session.booking.charger);
  }
